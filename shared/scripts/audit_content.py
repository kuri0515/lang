...
【試過但撤掉的檢查，別再做一次】
  「zh 與 note 裡的『＝』解釋對不上」——
  想法是抓 주문내역 那種矛盾（zh 寫「訂單紀錄」、note 寫「＝明細」）。
  實測全庫 73 命中，只有 1 條是真衝突，其餘全是 note 在解釋例句裡的其他詞
  （택배 的 note 解釋「택배가 왔어요＝包裹到了」）。
  信噪比 1/73，留著只會變成背景噪音，所以撤掉。
  這類「note 說明的對象不是詞條本身」的情況無法用規則分辨，需要人眼。
"""
...
OK_BOTH = {("台", "臺"), ("祕", "秘"), ("秘", "祕"),
           ...
           ("占", "佔"),
           # 拐→柺 · 背→揹  台灣就寫「拐杖」「背包」，OpenCC 轉的是異體字
           ("拐", "柺"), ("背", "揹"),
...
# 【日文站專屬】日文漢字的正式字形，OpenCC 會把它們當成簡體。
#
# 假名的字源說明必須寫日文的字形 —— 片假名 レ 來自「礼」，
# 轉成「禮」就把字源講錯了：學習者看的是「這個假名長得像哪個漢字」，
# 而它像的是 礼 不是 禮。
#
# ★ 只在日文站放行。韓文站的內容若出現「无」「尔」，那就是真的簡體，
#   兩站共用一份白名單會讓韓文站漏掉真問題。
JP_ONLY = {("礼", "禮"), ("无", "無"), ("尔", "爾"), ("祢", "禰"),
           ("踊", "踴"), ("搜", "蒐")}
if SITE == "japanese":
    OK_BOTH = OK_BOTH | JP_ONLY
...
def check_note_repeats(items):
    ...
    ★ 含記憶口訣的 note 直接跳過。日文站的假名說明是這樣寫的：
      「え･え･え！元･元･元，是誰A了我衣服裡的一百元！」——
      重複是記憶法的核心手段，不是貼過頭。
      判準取「同一段裡出現 X･X･X 或 X、X、X 這種三連」，
      那是口訣的固定形態，一般說明不會這樣寫。
    """
    chant = re.compile(r"(.)[･、,]\1[･、,]\1|(..)！\2")
    ...
    bad = []
    for x in items:
        note = (x.get("note") or "").replace(" ", "")
        if chant.search(note):
            continue                      # 記憶口訣，重複是刻意的
        d = dup(note)
        if d:
