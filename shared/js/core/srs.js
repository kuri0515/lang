...
export function schedule(card, rating, now = new Date(), opts = {}) {
  ...
  //
  // 【「有點難」為什麼是退回第一階，不是原地】
  //   這是刻意的改變。原本是原地，理由是「退階會讓一張詞在
  //   3→2→3→2 之間來回，永遠畢不了業」——那個顧慮沒有消失，
  //   但原地的問題更大：「有點難」與「記得」的差別只有一階，
  //   而人在猶豫時按哪一個幾乎是隨機的。
  //   退回第一階讓這個按鈕真的有意義：你說難，它就當你還沒學會。
  //
  //   代價要講清楚：一個 55 天的詞按一次「有點難」會掉回 1 天。
  //   所以「有點難」的定義要窄 —— 是「想了很久才想起來」，
  //   不是「有一點不確定」。介面上的說明要跟著改，否則
  //   使用者會用「有點難」表達「我不太有把握但答對了」，
  //   那個詞就永遠爬不上去。
  const step = rating === RATING.EASY ? 2 : 1;
  interval = rating === RATING.HARD
    ? LADDER[0]
    : LADDER[Math.min(TOP, rungOf(interval) + step)];
