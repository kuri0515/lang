...
# ★ 這個數字是**量出來的，而且量過兩次**。
#
#   第一次設 4 秒是猜的：第 1 集 45 幕裡有 22 幕正好卡在行數上限，
#   也就是「照停頓切幕」根本沒發生，切出來的是等長的塊。
#   量過才知道行間空檔中位數只有 0.6 秒，於是改成 2 秒。
#
#   第二次是四集都做完之後。2 秒剛好卡在「硬切占比」的警報線上
#   （25%），四集裡有兩集會叫 —— 而一個會被忽略的警報比沒有警報更糟。
#   實測四集：
#     1.2s → 硬切  9%，平均 4.2 行/幕（太碎）
#     1.5s → 硬切 14%，平均 5.4 行/幕  ← 選這個
#     2.0s → 硬切 26%，平均 6.5 行/幕（警報線上）
#     2.5s → 硬切 35%，平均 7.6 行/幕
#   1.5 秒讓切幕真的由停頓決定，而不是被上限截斷。
#
# 【改這個數字的代價】
#   幕號會全部重排，而閱讀進度（script_progress）是按幕號存的 ——
#   改完之後舊的標記會指到不同的內容。
#   所以要改就趁早：這次改時只有 1 筆標記、0 個精讀輪次池。
#   集數多了之後就不該再動它了。
SCENE_GAP = 1.5
...
def build_rules():
    ...
    # ── 第二批（2026-08）──────────────────────────────────────
    # 覆蓋率停在 37%，而漏掉的多半不是罕見句型，是**最基本的那些**：
    # 過去、斷定、否定、疑問、終助詞。第一版刻意不標它們，
    # 是怕像 の・が 那樣誤判；但這幾個看詞性就分得出來，不必猜。
    #
    # 仍然守「寧可漏，不可錯」：分不出來的一律不標。
    # 所以命令形沒有加 —— Sudachi 這裡只給到詞形與詞性，拿不到活用形，
    # 想靠字尾猜（〜ろ／〜え）會把「面白いから」這種也咬進來。

    # 過去・完了。助動詞的 た 才算，「〜たり」「〜たら」有各自的規則
    "ta":      lambda w: any(x[2]=='助動詞' and x[1]=='た' for x in w),
    # 斷定。★ 不能只看詞形是不是「だ」——
    #   Sudachi 把「確かに」的「に」也標成詞形 だ 的助動詞（形容動詞的連用形），
    #   而「だが」「だから」「だって」是接續詞，不是斷定。
    #   兩種都會讓學習者在一個沒有斷定的句子上讀到「斷定」的說明。
    #   所以看**表面形**，並排除後面接 が／から／って 的那些。
    "da-desu": lambda w: any(
        x[2]=='助動詞' and x[0] in ('だ','です','だっ','でし','じゃ','では','だろ','でしょ')
        and not (x[0]=='だ' and i+1 < len(w) and w[i+1][0] in ('が','から','って'))
        for i, x in enumerate(w)),
    # 否定。ない／ぬ（〜せんか 的 ん 也是這個）
    "nai":     lambda w: any(x[2]=='助動詞' and x[1] in ('ない','ぬ') for x in w),
    # 疑問。か 當助詞才算 —— 「か」也可能是名詞的一部分
    "ka":      lambda w: any(x[2]=='助詞' and x[1]=='か' for x in w),
    # 終助詞的語氣：よ（告知）ね（確認）
    "yo-ne":   lambda w: any(x[2]=='助詞' and x[3]=='終助詞' and x[1] in ('よ','ね') for x in w),
    # ぞ・ぜ：粗獷、男性化的語氣。這部作品裡它是角色的說話方式，不是隨機的
    "zo-ze":   lambda w: any(x[2]=='助詞' and x[1] in ('ぞ','ぜ') for x in w),
    # わ：柔和／驚訝的語氣（關西腔也用，但不分性別都是語氣詞）
    "wa-shuu": lambda w: any(x[2]=='助詞' and x[3]=='終助詞' and x[1]=='わ' for x in w),
    # んだ／のだ：說明・強調。★ 一定要看前一個詞是不是述語 ——
    # 不看的話，所有格的「の」會被算進來（第一版實測誤中 109 行）
    "n-da":    lambda w: any(w[i][1]=='ん' and w[i][2]=='助詞'
                             and i+1 < len(w) and w[i+1][1] in ('だ','です')
                             for i in range(len(w))),
    # 引用：と＋言う／思う。單純的 と 不算（那是並列或條件）
    "to-iu":   lambda w: any(w[i][1]=='と' and w[i][2]=='助詞'
                             and i+1 < len(w) and w[i+1][1] in ('言う','いう','思う','聞く','考える')
                             for i in range(len(w))),
    # って：口語的引用與提題。書面體不會出現，正是看劇才學得到的東西。
    # ★ 前面是助動詞「だ」時不算 —— 那是接續詞「だって（因為／可是）」，
    #   Sudachi 會把它切成 だ＋って，而那句話裡根本沒有引用。
    "tte":     lambda w: any(x[2]=='助詞' and x[1]=='って'
                             and not (i and w[i-1][2]=='助動詞' and w[i-1][0]=='だ')
                             for i, x in enumerate(w)),
    # 敬稱：さん／ちゃん／君／様 的分寸。這部作品裡它直接反映人物關係
    "keishou": lambda w: any(x[2]=='接尾辞' and x[1] in ('さん','ちゃん','君','様','くん') for x in w),
    # 〜すぎる：過度
    "sugiru":  lambda w: has(w,1,'すぎる','過ぎる'),
    }
    return R
