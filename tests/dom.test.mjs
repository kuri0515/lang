...
// 【開站失敗時，畫面要自己說出原因】
//
// 模組在載入或求值階段爆掉的話整頁是白的，而手機上沒有主控台 ——
// 使用者能回報的只有「打不開」，那句話裡沒有任何可以下手的資訊。
// （2026-08-19 真的發生了：手機打不開，而遠端能查的東西全是綠的。）
// =====================================================================
{
  const html = fs.readFileSync(`${SITE_DIR}/index.html`, 'utf8');
  const i = html.indexOf('unhandledrejection');
  const j = html.indexOf('<script type="module"');
  chk('★ 有攔開站錯誤', i > 0, '白畫面不說原因，等於把除錯成本推給使用者');
  chk('★ 攔截器排在模組之前', i > 0 && j > 0 && i < j,
      '排在後面的話，最早、也最要緊的那個錯誤攔不到');
  chk('攔截器不是 module', /<script>\s*\(function/.test(html),
      'type=module 會被延後執行 —— 那時要攔的錯誤已經發生了');
  chk('錯誤畫面會帶上版本與 UA', html.includes('meta[name=build]') && html.includes('userAgent'),
      '沒有版本就分不清是新 bug 還是舊快取');
}
